...
def working(text=text):
    ...
    return (conversion_prediction, likes_prediction,
        shap.force_plot(shap_explainer.expected_value, shap_values, X, feature_names=feature_names_all), tok_without_names_unique
    )
...
if st.button("""✨ Угадать популярность"""):
  ...
  <big><span style="color: Coral;">Пожалуйста введите текст :)</span></big>
    ...
    if output[0] < 0.68:
        ...
        st.markdown(f"""Конверсия этого текста в лайки или другими словами, <b>его популярность</b>)- составляет <i><font size="+4"><span style="color: Coral;">""" + conversion + """%""" + """</span></font></i>. Это неплохой результат - по результатам анализа он попадает в <b>45%</b>, скажем так, "добротных" текстов. Если бы он был опубликован, то получил бы примерно <i><font size="+4"><span style="color: Coral;">""" + likes + """</span></font></i> лайков. \n \n Что повлияло на предсказание? Попробуем разобраться с помощью диаграммы ниже.""", unsafe_allow_html=True)
    elif output[0] >= 2.2 and output[0] < 3.8:
        st.markdown(f"""Конверсия этого текста в лайки или проще говоря, <b>его популярность</b>)- составляет <i><font size="+4"><span style="color: Coral;">""" + conversion + """%""" + """</span></font></i>. Что-ж, можно считать, это <b>хороший</b> текст. По результатам анализа он в числе <b>25%</b> качественных постов, собравших немало лайков. Если бы этот текст был опубликован, то получил бы примерно <i><font size="+4"><span style="color: Coral;">""" + likes + """</span></font></i> лайков. \n \n Что повлияло на предсказание? Попробуем разобраться с помощью диаграммы ниже.""", unsafe_allow_html=True)
    else:
    ...
    #st.markdown(output[3], unsafe_allow_html=True)
...
st.markdown("""<font size="+4">Подробнее о признаках</font>    
...
st.markdown("""Для достижения повышения конверсии просмотров в лайки также можно ориентироваться на подходящее время публикации. Видно, что посты опубликованные в понедельник и в середине недели в среднем популярнее. Также больше поддержки получают тексты, опубликованные утром/в первой половине дня.   
